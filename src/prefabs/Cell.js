class Cell {
    ...
    * */
    ...
    addPlant(plantType, plantX, plantY, plantGrowthLevel) {
        if (this.dataView) {
            this.removePlant();
            let plant = new Plant(this.scene, plantX, plantY, new DataView(dataView.buffer, 2, 4));
    ...
    saveData() {
        if (this.dataView) {
            let data = {};
            if (this.plantGrowthLevel != 3) { //not null
                //data.plant = this.plant.saveData(); does not work since it cant actually call the plant
                data.plant = { x: this.plantX, y: this.plantY, type: this.plantType, growthLevel: this.plantGrowthLevel };
            }
    ...
    loadData(data) {
        if (this.dataView) {
            ...
            if (this.plant) {
                this.addPlant(this.plantType, this.plantX, this.plantY, this.plantGrowthLevel);
            }
        }
    }
